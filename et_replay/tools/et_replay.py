...
class ExgrReplayManager:
    def __init__(self):
        ...
        # Checkpoint timing (warmup iterations excluded)
        self.checkpoint_total_time_ms = 0.0
        self.checkpoint_count = 0

        # Raw execution trace JSON (optional). When available, this allows
        # us to read custom attributes that are not mapped onto Node objects,
        # such as checkpoint_total_storage_bytes injected by the Chakra linker.
        self.raw_trace: Optional[dict] = None
        ...
        if not self.args.trace_path:
            ...
            if "://" in self.args.input:
                try:
                    ...
                    raw_trace = json.load(et)
                    self.et = ExecutionTrace(raw_trace)
                    # Keep raw JSON so we can access custom attributes (e.g., checkpoint sizes).
    ...
    def extract_subgraph(self, root):
        ...
        def analayze_node(node):
            ...
            # The output tensors from the comm nodes also need to be allocated. Since 
            ...
            if (
                ...
                and node.name
                and node.name.upper().startswith("CHECKPOINT")
            ):
                return True, ""
        ...
        def dfs_traverse(node):
            ...
            if node.type == NodeType.OPERATOR:
                ...
                return
            # Also add checkpoint label nodes to sorted_nodes
            if (
                node.type == NodeType.LABEL
                and node.name
                and node.name.upper().startswith("CHECKPOINT")
    ...
    def run_op(self, node, iter, cnt):  # noqa: C901
        ...
        if isinstance(node, commsArgs):
            ...
            # Handle checkpoint nodes: simulate checkpoint save by writing dummy file
            if (
                node.type == NodeType.LABEL
                and node.name
                and node.name.upper().startswith("CHECKPOINT")
            ):
                # Get checkpoint metadata from raw trace
                checkpoint_path = None
                checkpoint_size = 0
                
                if self.raw_trace:
                    raw_node = next(
                        (n for n in self.raw_trace.get("nodes", []) if n.get("id") == node.id),
                        None
                    )
                    if raw_node:
                        for attr in raw_node.get("attrs", []):
                            if attr.get("name") == "checkpoint_path":
                                checkpoint_path = attr.get("value")
                            elif attr.get("name") == "checkpoint_total_storage_bytes":
                                checkpoint_size = int(attr.get("value", 0))
                
                # Write dummy checkpoint file if we have path and size
                if checkpoint_path and checkpoint_size > 0:
                    # Get output directory (default to ./checkpoints)
                    output_dir = getattr(self.args, "checkpoint_output_dir", "./checkpoints")
                    os.makedirs(output_dir, exist_ok=True)
                    
                    # Extract filename from checkpoint_path
                    ...
                    try:
                        ...
                        # Instead of just writing zeros, simulate actual serialization
                        import pickle
                        
                        # Create dummy checkpoint structure matching real checkpoint
                        dummy_checkpoint = {
                            'epoch': 0,
                            'step': 0,
                            'batch_size': 32,
                        ...
                        # here we make assumption that the checkpoint size is split evenly between model and optimizer state
                        target_data_size = int(checkpoint_size)
                        model_state_target = checkpoint_size // 2  
                        optimizer_state_target = checkpoint_size // 2  
                        
                        # Estimate bytes per numpy array when pickled 1Mb 
                        bytes_per_array_raw = 1024 * 1024  
                        bytes_per_array_serialized = int(bytes_per_array_raw)
                        
                        # Calculate number of arrays needed for each dict
                        num_model_arrays = max(1, model_state_target // bytes_per_array_serialized)
                        num_optimizer_arrays = max(1, optimizer_state_target // bytes_per_array_serialized)
                        
                        # Add dummy numpy arrays to model_state_dict
                        for i in range(num_model_arrays):  # Cap at 100 arrays
                            array_size = bytes_per_array_raw // 4  # FP32 = 4 bytes per element
                            # dummy_checkpoint['model_state_dict'][f'layer_{i}.weight'] = np.zeros(array_size, dtype=np.float32)
                            dummy_checkpoint['model_state_dict'][f'layer_{i}.weight'] = np.random.randn(array_size).astype(np.float32)
                            
                        
                        # Add dummy numpy arrays to optimizer_state_dict
                        # Optimizer state typically has similar structure (momentum buffers)
                        for i in range(num_optimizer_arrays):  # Cap at 100 arrays
                            array_size = bytes_per_array_raw // 4  # FP32 = 4 bytes per element
                            # dummy_checkpoint['optimizer_state_dict'][f'layer_{i}.weight'] = {
                            #     'momentum_buffer': np.zeros(array_size, dtype=np.float32)
                            # }
                            dummy_checkpoint['optimizer_state_dict'][f'layer_{i}.weight'] = {
                                'momentum_buffer': np.random.randn(array_size).astype(np.float32)
                            }
                        
                        # Actually serialize it (this will take CPU time like real torch.save)
                        start_serialize = time.time()
                        serialized = pickle.dumps(dummy_checkpoint, protocol=pickle.HIGHEST_PROTOCOL)
                        ...
                        # Accumulate checkpoint timing (skip warmup iterations)
                        if iter >= self.numWarmupIters:
                            self.checkpoint_total_time_ms += (time.time() - cp_start) * 1000.0
                            self.checkpoint_count += 1

                        logger.info(
                            f"Simulated checkpoint save: {output_path} ({checkpoint_size} bytes, "
                            f"serialization: {serialize_time*1000:.2f}ms)"
                        )
                    except OSError as e:
                        logger.warning(f"Failed to write checkpoint file {output_path}: {e}")
    ...
    def benchTime(self):
        ...
        if self.checkpoint_count > 0:
            avg_cp = self.checkpoint_total_time_ms / self.checkpoint_count
            logger.info(
                f"Checkpoint avg time: {avg_cp:.2f} ms over {self.checkpoint_count} checkpoints"
            )
        logger.info(
    ...
    def readComputeArgs(self, check_args: bool = True):
        ...
        parser.add_argument(
            ...
            type=str,
            default="./checkpoints",
            help="Directory to write simulated checkpoint files during replay. Defaults to ./checkpoints.",
        )
        parser.add_argument(
