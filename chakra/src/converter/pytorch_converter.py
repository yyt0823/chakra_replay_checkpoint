...
class PyTorchConverter:
    ...
    def convert(
        ...
        output_filename: str,
        simulate: bool,
        checkpoint_metadata_file: Optional[str] = None,
    ) -> None:
        """
        ...
        Args:
            ...
            checkpoint_metadata_file (Optional[str]): Deprecated. Checkpoint metadata is now read directly from
                node attributes in the linked trace. This parameter is kept for backward compatibility but ignored.
        """
        if checkpoint_metadata_file:
            logging.warning(
                "The --checkpoint-metadata parameter is deprecated. Checkpoint metadata is now read directly "
                "from node attributes in the linked trace. The provided file will be ignored."
            )
        ...
        protobuf_node_map = {}
        self.convert_json_to_protobuf_nodes(
            json_node_map,
            protobuf_node_map,
        )
    ...
    def convert_json_to_protobuf_nodes(
        ...
        for _, json_node in json_node_map.items():
            if (
                ...
                chakra_node = self.convert_json_to_protobuf_node(
                    json_node_map,
                    protobuf_node_map,
                    json_node,
                )
    ...
    def convert_json_to_protobuf_node(
        ...
        # Read checkpoint metadata directly from node attributes (added by trace_linker)
        if json_node.is_checkpoint_op():
            # Checkpoint metadata attributes are already attached by trace_linker
            checkpoint_attrs = [
                "checkpoint_path",
                "checkpoint_epoch",
                "checkpoint_step",
            ...
            for attr_name in checkpoint_attrs:
                if hasattr(json_node, attr_name):
                    attr_value = getattr(json_node, attr_name)
                    if attr_value is not None:
                        try:
                            if attr_name == "checkpoint_path":
                                protobuf_node.attr.append(
                                    ChakraAttr(name=attr_name, string_val=str(attr_value))
                                )
                            else:
                                protobuf_node.attr.append(
                                    ChakraAttr(name=attr_name, int64_val=int(attr_value))
                                )
                        except (ValueError, TypeError) as e:
                            logging.debug(
                                "Skipping checkpoint attribute %s=%s: %s",
                                attr_name,
                                attr_value,
                                e,
                            )
    ...
    def _match_checkpoint_metadata(
        self,
        json_node_map: Dict[int, PyTorchNode],
        json_node: PyTorchNode,
        checkpoint_metadata: Dict[str, List[Dict]],
    ) -> Optional[Dict]:
        """
        Match a checkpoint node with its metadata entry using the enclosing ProfilerStep.
        """

        if not checkpoint_metadata:
            return None

        step = self._extract_profiler_step(json_node_map, json_node)
        if step is None:
            return None

        for path, entries in checkpoint_metadata.items():
            for entry in entries:
                entry_step = entry.get("step")
                if entry_step is None:
                    continue
                try:
                    if int(entry_step) == int(step):
                        entry.setdefault("checkpoint_path", path)
                        return entry
    ...
    def _extract_profiler_step(
        self, json_node_map: Dict[int, PyTorchNode], json_node: PyTorchNode
    ) -> Optional[int]:
        """
        Traverse parent relationships to find the enclosing ProfilerStep#X and return X.
        """

        def normalize(parent_field):
            if parent_field is None:
                return []
            if isinstance(parent_field, int):
                return [parent_field]
            if isinstance(parent_field, list):
                return [pid for pid in parent_field if isinstance(pid, int)]
            return []

        visited: Set[int] = set()
        stack = normalize(json_node.parent)
        ...
        while stack:
            ...
            parent_node = json_node_map.get(parent_id)
            if not parent_node:
                continue

            parent_name = parent_node.name or ""
            if parent_name.startswith("ProfilerStep#"):
                try:
                    return int(parent_name.split("#", 1)[1])
                except (ValueError, IndexError):
                    return None

            stack.extend(normalize(parent_node.parent))
