...
def convert_pytorch(args: argparse.Namespace) -> None:
    ...
    converter.convert(
        ...
        args.output,
        args.simulate,
        checkpoint_metadata_file=getattr(args, "checkpoint_metadata", None),
    )
...
def main() -> None:
    ...
    pytorch_parser.add_argument(
        "--checkpoint-metadata",
        type=str,
        help="Path to checkpoint metadata JSON (e.g., checkpoint_metadata_<rank>.json) to enrich checkpoint nodes.",
    )
