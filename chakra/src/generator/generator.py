...
def one_checkpoint_node(num_npus: int, step: int, epoch: int, batch_size: int, checkpoint_path: str) -> None:
    """Generate checkpoint_node."""

    """ next step : encode the weight and bias tensor into this ??? coule be huge. """
    """ current idea : find where thoes data are stored and return a path """
    for npu_id in range(num_npus):
        output_filename =  f"one_checkpoint_node.{npu_id}.et"
        with open(output_filename, "wb") as et:
            encode_message(et, GlobalMetadata(version="0.0.4"))

            node = get_node("CHECKPOINT_NODE", CHECKPOINT_NODE)
            node.attr.append(ChakraAttr(name="is_cpu_op", bool_val=False))
            node.attr.append(ChakraAttr(name="step",int64_val=step))
            node.attr.append(ChakraAttr(name="epoch",int64_val=epoch))
            node.attr.append(ChakraAttr(name="batch_size",int64_val=batch_size))
            node.attr.append(ChakraAttr(name="checkpoint_path", string_val=checkpoint_path))
