...
class TraceLinker:
    ...
    def link(
        ...
        output_file: str,
        checkpoint_metadata_path: Optional[str] = None,
    ) -> None:
        """
        Links Chakra host execution traces (ET) and Chakra device ET to generate Chakra host + device ET.
        ...
        # Attach checkpoint metadata to checkpoint nodes if provided
        if checkpoint_metadata_path is not None:
            try:
                with open(checkpoint_metadata_path, "r") as f:
                    checkpoint_metadata = json.load(f)

                nodes = chakra_execution_trace_plus_data.get("nodes", [])

                # Build a quick id -> node map so we can follow ctrl_deps chains
                node_by_id = {}
                for n in nodes:
                    nid = n.get("id")
                    if nid is not None:
                ...
                def find_profiler_step_index(start_node) -> Optional[int]:
                    """
                    Walk up the control-dependency chain from this node until we find
                    a node whose name looks like 'ProfilerStep#<k>'. Return k if found.
                    """
                    visited = set()
                    ctrl_deps = start_node.get("ctrl_deps", [])
                    # Handle case where ctrl_deps might be a single int instead of a list
                    if isinstance(ctrl_deps, int):
                        stack = [ctrl_deps]
                    elif isinstance(ctrl_deps, list):
                        stack = list(ctrl_deps)
                    else:
                        stack = []
                    ...
                    while stack:
                        ...
                        # Keep walking further up the control-dependency chain
                        parent_ctrl_deps = parent.get("ctrl_deps", [])
                        if isinstance(parent_ctrl_deps, int):
                            stack.append(parent_ctrl_deps)
                        elif isinstance(parent_ctrl_deps, list):
                            stack.extend(parent_ctrl_deps)

                    return None

                # 1) Flatten metadata entries, filter by rank, and sort by (epoch, step, rank)
                flat_entries: list[tuple[str, dict]] = []
                if isinstance(checkpoint_metadata, dict):
                    for ckpt_path, meta_list in checkpoint_metadata.items():
                        # Handle case where meta_list might not be a list
                        if not isinstance(meta_list, list):
                            logging.warning(
                                "Skipping checkpoint path '%s': expected list but got %s",
                                ckpt_path,
                                type(meta_list).__name__,
                            )
                            continue
                        for meta in meta_list:
                            if isinstance(meta, dict):
                                # Only include metadata entries matching the current rank
                                meta_rank = meta.get("rank")
                                if meta_rank is not None and meta_rank == rank:
                                    flat_entries.append((ckpt_path, meta))
                ...
                # 2) Collect all CHECKPOINT_Save nodes and compute their ProfilerStep index
                checkpoint_nodes_with_idx = []
                for n in nodes:
                    if n.get("name") == "CHECKPOINT_Save":
                        step_idx = find_profiler_step_index(n)
                        checkpoint_nodes_with_idx.append((step_idx, n))

                # Sort by ProfilerStep index (None goes last)
                checkpoint_nodes_with_idx.sort(
                    key=lambda pair: (pair[0] if pair[0] is not None else 1_000_000_000)
                )
                checkpoint_nodes = [n for (_, n) in checkpoint_nodes_with_idx]

                # 3) Validate and pair nodes with metadata
                if len(checkpoint_nodes) != len(flat_entries):
                    logging.warning(
                        "Number of CHECKPOINT_Save nodes (%d) does not match number of "
                        "checkpoint metadata entries for rank %d (%d). Will pair in order and ignore extras.",
                        len(checkpoint_nodes),
                        rank,
                        len(flat_entries),
                    )

                # Pair nodes with metadata (only pair up to the minimum length to avoid errors)
                min_length = min(len(checkpoint_nodes), len(flat_entries))
                for i in range(min_length):
                    node = checkpoint_nodes[i]
                    ckpt_path, meta = flat_entries[i]
                    attrs = node.setdefault("attrs", [])

                    def add_attr(name, attr_type, value):
                        attrs.append(
                            {
                                "name": name,
                                "type": attr_type,
                    ...
                    add_attr("checkpoint_path", "string", ckpt_path)
                    add_attr("checkpoint_epoch", "int64", int(meta.get("epoch", 0)))
                    add_attr("checkpoint_step", "int64", int(meta.get("step", 0)))
                    add_attr("checkpoint_rank", "int64", int(meta.get("rank", 0)))
                    add_attr("checkpoint_batch_size", "int64", int(meta.get("batch_size", 0)))
                    add_attr(
                        "checkpoint_total_storage_bytes",
                        "uint64",
                        int(meta.get("total_storage_bytes", 0)),
                    )

                if min_length > 0:
                    logging.info(
                        "Attached checkpoint metadata to %d CHECKPOINT_Save nodes for rank %d.",
                        min_length,
                        rank,
                    )
                ...
                logging.warning(
                    "Failed to load or attach checkpoint metadata from %s: %s",
